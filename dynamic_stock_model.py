...
class DynamicStockModel(object):
    ...
    def __init__(self, t=None, i=None, i_c=None, o=None, s=None, lt=None, s_c=None, o_c=None, name='DSM', pdf=None, sf=None):
        """ Init function. Assign the input data to the instance of the object."""
        self.t = t  # optional

        self.i = i  # optional
        self.i_c = i_c # Added by Fernando
    ...
    def compute_s_c_from_i_c(self): #Added by Fernando
        ...
        if self.i is not None:
            if self.lt is not None:
                self.compute_sf()
                self.s_c = np.einsum('tc,tc->tc', self.i, self.sf) # See numpy's np.einsum for documentation.
                # This command means: s_c[t,c] = i[c] * sf[t,c] for all t, c
                # from the perspective of the stock the inflow has the dimension age-cohort, 
                # as each inflow(t) is added to the age-cohort c = t
                return self.s_c
    ...
    def compute_o_c_from_s_c_with_i_c(self):
        """Compute outflow by cohort from stock by cohort."""
        if self.s_c is not None:
            if self.o_c is None:
                self.o_c = np.zeros(self.s_c.shape)
                self.o_c[:,1:] = -1 * np.diff(self.s_c.T,n=1,axis=1)
                self.o_c[np.diag_indices(len(self.t))] = np.diag(self.i_c) - np.diag(self.s_c) # allow for outflow in year 0 already
                #self.o_c = self.i_c - self.s_c
                return self.o_c
            else:
                # o_c already exists. Doing nothing.
                return self.o_c
    ...
    def compute_i_c_from_s_c(self, InitialStock):
        """Given a stock at t0 broken down by different cohorts tx ... t0, an "initial stock". 
           This method calculates the original inflow that generated this stock.
           Example: 
        """
        if self.i_c is None: # only in cases where no inflow has been specified.
            if len(InitialStock) == len(self.t):
                self.i_c = np.zeros((len(self.t), len(self.t)))
                # construct the sf of a product of cohort tc surviving year t 
                # using the lifetime distributions of the past age-cohorts
                self.compute_sf()
                for Cohort in range(0, len(self.t)):
